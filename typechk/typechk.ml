...
let bin_oper_int e1 e2 =
	let (t1,t2) = (getType e1, getType e2) in
		t1 = TNumber && t2 = TNumber
	

let bin_oper_str e1 e2 =
	let (t1,t2) = (getType e1, getType e2) in
		t1 = TString && t2 = TString


let bin_oper_bool e1 e2 =
	let (t1,t2) = (getType e1, getType e2) in
		t1 = TBoolean && t2 = TBoolean
...
let type_of_array a = 
	match a with
		| TArray(_,t) -> t
		| _ -> raise (Invalid_type "Array expected")
...
let rec typechkExp env e =
	match e with
		...
		(* Este match esta mal porque o t que vem do parser e Undefined *)
		(* logo deve-se usar wild card *)
		(* e preciso usar um fold para verificar e correr o type check *)
		(* para cada um *)
		| Array(l,t) -> let size = List.length l in
  										List.iter(fun next -> 
            												let t' = getType next in
              												if t' <> t then 
          															raise (Invalid_type "array") 
          											) l ; 
											Array(l,TArray(size,t))
		...
		| Add (e1, e2, _) -> let (e1',e2') = (typechkExp env e1,typechkExp env e2) in
													if (bin_oper_int e1' e2') then 
														Add(e1', e2', TNumber)
													else if (bin_oper_str e1' e2') then 
														Add(e1', e2', TString)
													else 
														raise (Invalid_type "")
		...
		| Eq (e1, e2, _) -> let (e1',e2') = (typechkExp env e1,typechkExp env e2) in
													if (bin_oper_int e1' e2') || (bin_oper_str e1' e2') then 
														Eq(e1',e2',TBoolean)
													else 
														raise (Invalid_type "")
		...
		| GetArray(e1, e2, _) -> let(e1',e2') = (typechkExp env e1,typechkExp env e2) in 
															let t1 = getType e1' in
																if (un_oper_array t1) && (un_oper_int e2') then( 
																	let ty_array = type_of_array t1 in GetArray(e1',e2',ty_array)
																)else 
																	raise (Invalid_type "")
